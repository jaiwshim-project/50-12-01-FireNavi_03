...
(function () {
  ...
  const isInLab = /\/lab\//.test(path);
  const pre = isSubpage ? '../' : '';
  // Lab(Mobile/site/lab/) 안에서는 Mobile/ 루트까지 ../../ 로 올라가야 함
  const labRoot = isInLab ? '../../' : pre;
  // v3.0 20장 피치 덱은 Mobile/firenavi-v3-slides.html에 위치
  const slidesHref = isInLab ? '../../firenavi-v3-slides.html' : (pre + 'pitch-deck.html');
    ...
    <footer class="footer-premium">
      <div class="footer-premium-inner">
        ...
        <div class="footer-cta-row" style="display:flex;justify-content:center;gap:14px;flex-wrap:wrap;margin:32px 0 20px;">
          <a href="${slidesHref}" target="${isInLab ? '_blank' : '_self'}" class="footer-cta footer-cta--primary"
